...
const TaskCard = ({task, toggleFavorite, toggleComplete, deleteToDo, updateToDo }) => {
    return(
        <Container style={{textAlign:"center",boxShadow:"5px 5px 5px 5px pink",marginTop:"20px",padding:"20px",borderRadius:"8px",color:"white",backgroundColor:"lightblue"}}>
            <Row>
                <Col>{task.favorite? <StarFill onClick={e=> toggleFavorite(e, task._id)} size={23} />: <Star size={23} onClick={e=> toggleFavorite()}/>}</Col>
            </Row>
            <Row>
                <Col>
                    <Form.Check type="checkbox" checked={task.complete} onChange={e => toggleComplete(e, task._id) }/>
                </Col>
                <Col>
                ...
                <Col>
                    ...
                    <XCircle size={23} onClick={ e => deleteToDo(e, task._id) }/>
                </Col>
            </Row>
            <Row>
                Tags: 
